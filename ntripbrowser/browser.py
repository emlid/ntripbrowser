...
def argparser():
    ...
    parser.add_argument("-t", "--timeout", type=int, help="add timeout", default=4)
    parser.add_argument("-c", "--coordinates", help="Add NTRIP station distance to this coordinate", nargs=2)
    parser.add_argument(
        "-M",
        "--maxdist",
...
def main():
    ...
    browser = NtripBrowser(
        args.url, port=args.port, timeout=args.timeout, coordinates=args.coordinates, maxdist=args.maxdist
    )
    try:
